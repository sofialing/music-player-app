...
const PlaybackContextProvider = ({ children }) => {
	...
	useEffect(() => {
		if (user && user?.is_premium && !scriptLoaded) {
			loadSpotifyScript();
		}

		window.onSpotifyWebPlaybackSDKReady = () => {
			// add title to iframe generated by Spotify Web Playback SDk to improve accessibility
			if (document.querySelector('iframe')) {
				document.querySelector('iframe').setAttribute('title', 'Audio playback container');
			}
			...
			// Error handling
			player.addListener('initialization_error', ({ message }) => console.log('Failed to initialize', message));
			player.addListener('authentication_error', ({ message }) => console.log('Failed to authenticate', message));
			player.addListener('account_error', ({ message }) => console.log('Failed to validate Spotify account', message));
			player.addListener('playback_error', ({ message }) => console.log('Failed to perform playback', message));
			...
			player.addListener('ready', ({ device_id }) => dispatch({ type: 'SET_DEVICE_ID', device_id }));

			// Connect the Web Playback SDK instance to Spotify
			player.connect().then(success => {
				if (success) dispatch({ type: 'SET_PLAYER', player })
			});
		};
	}, [user, scriptLoaded])
